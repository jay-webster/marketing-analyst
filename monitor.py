...
def send_update_email(company_name, change_summary, deep_dive_url=None):
    ...
    <div style="max-width: 600px; margin: 0 auto; border: 1px solid #e0e0e0; border-radius: 8px; overflow: hidden;">
        <div style="background-color: #0044cc; color: white; padding: 20px;">
            <h2 style="margin: 0; font-size: 20px;">🚨 Competitor Alert: {company_name}</h2>
        </div>
        ...
        <div style="background-color: #f8f9fa; padding: 15px; text-align: center; color: #888; font-size: 11px;">
            Generated by Marketing Analyst Agent • {datetime.now().strftime('%Y-%m-%d')}
        </div>
    </div>
    </body></html>
    """
    for email in recipients:
        utils.send_email(
            subject=f"⚠️ Strategy Update: {company_name}",
...
def send_baseline_report(new_subscriber_email):
    ...
    try:
        ...
        for doc in track_docs:
            ...
            news_html = ""
            news_html += format_section("🚀 Launches", news_data.get("launches"))
            news_html += format_section(
                "🤝 Partnerships", news_data.get("partnerships")
            )
            news_html += format_section("👔 Leadership", news_data.get("leadership"))
            news_html += format_section("💰 Funding", news_data.get("funding"))
        ...
        <body style="font-family: 'Helvetica Neue', Arial, sans-serif; color: #333; background-color: #f4f4f4; padding: 40px 20px;">
            <div style="max-width: 700px; margin: 0 auto;">
                <div style="text-align: center; margin-bottom: 40px;">
                    ...
                    <p style="margin: 10px 0 0 0; color: #7f8c8d; font-size: 14px;">Active Surveillance Report • {datetime.now().strftime('%B %d, %Y')}</p>
                </div>
                {competitor_cards}
...
async def analyze_competitor_website(domain):
    print(f"🔎 [Direct-Fetch] Starting Analysis for {domain}...")
    ...
    try:
        ...
        print(f"❌ Analysis Parse Error (First Attempt): {e}")

    # --- FALLBACK: MEMORY RECOVERY ---
    print(f"⚠️ Direct Analysis Failed. Attempting Memory Fallback for {domain}...")
    try:
        fallback_prompt = f"""
