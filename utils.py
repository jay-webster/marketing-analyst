...
def send_email(
    ...
    # If there is an attachment, we use "mixed".
    # If it is just text/html, we use "alternative" so the client can choose the best view.
    if pdf_bytes:
    ...
    # 2. Attach Plain Text (Always required as fallback)
    msg.attach(MIMEText(body_text, "plain"))

    # 3. Attach HTML (Optional)
    if html_body:
